...
class Batch extends \Bitrix24\SDK\Core\Batch
{
    ...
    public function deleteEntityItems(
        ...
        try {
            ...
            foreach ($entityItemId as $cnt => $itemId) {
                ...
                $this->registerCommand($apiMethod, ['taskId' => $itemId]);
            }
            ...
            $errorMessage = sprintf('batch delete tasks: %s', $exception->getMessage());
            $this->logger->error(
                $errorMessage,
                [
    ...
    public function updateEntityItems(string $apiMethod, array $entityItems): Generator
    {
        ...
        try {
            ...
            foreach ($entityItems as $entityItemId => $entityItem) {
                ...
                $cmdArguments = [];
                $cmdArguments['taskId'] = $entityItemId;
                $cmdArguments['fields'] = $entityItem;
    ...
    /**
     * Determines the ID key for Task API
     * Task API always uses lowercase 'id' regardless of parameters
     */
     ...
     * Task API returns data in 'tasks' key
     */
    ...
    {
        // Task API always uses 'tasks' key regardless of $isCrmItemsInBatch parameter
        return sprintf('$result[%s][tasks][%d][%s]', $prevCommandId, $lastIndex, $keyId);
    }
    ...
    protected function extractElementsFromBatchResult(ResponseData $responseData, bool $isCrmItemsInBatch = false): array
    {
        ...
        if (array_key_exists('tasks', $resultData)) {
